...
**`df.head(n)` / `df.tail(n)`**: 查看前n行或后n行，快速确认数据加载是否正确。

**`df.info()`**: **这是最重要的体检命令**。它展示行数、列数、每列的数据类型（dtype）以及非空值的数量。

**`df.describe()`**: 快速统计数值列的概况（均值、标准差、最大/最小值、分位数）。

**`df.shape`**: 返回一个元组 `(行数, 列数)`，用于查看数据规模。

**`df.columns` / `df.index`**: 分别获取列名列表和索引对象。

| **命令**            | **描述**                        | **示例**                           |
| ------------------- | ------------------------------- | ---------------------------------- |
| **`df.loc[]`**      | **基于标签（Label）**的索引。   | `df.loc[row_label, col_label]`     |
| **`df.iloc[]`**     | **基于位置（Integer）**的索引。 | `df.iloc[0:5, 0:2]` (前5行，前2列) |
| **`df['col']`**     | 选择单列（返回Series）。        | `df['price']`                      |
| **`df[condition]`** | **布尔索引**（条件筛选）。      | `df[df['age'] > 30]`               |

**`df.isnull()` / `df.isna()`**: 检查缺失值，通常配合 `.sum()` 使用来统计缺失数量。

**`df.dropna()`**: 直接删除包含缺失值的行或列（暴力法）。

**`df.fillna(value)`**: 用指定值（如均值、0）填充缺失值（温和法）。

**`df.drop_duplicates()`**: 删除重复行。

**`df.astype()`**: 强制转换数据类型（例如将字符串类型的数字转换为 `float`）。

**`df.rename()`**: 重命名列名或索引，调整架构的语义。

**`df.groupby()`**: **架构的核心**。类似于SQL的 `GROUP BY`，用于分组统计。

- *常用组合:* `df.groupby('category')['value'].sum()`

**`df.pivot_table()`**: 透视表。Excel用户的最爱，用于多维度汇总数据。

**`df.sort_values()`**: 根据值进行排序。

**`df.apply()`**: 将自定义函数应用到DataFrame的行或列上，极大地扩展了处理能力。

**`pd.merge()`**: 类似于SQL的 `JOIN`。支持 `left`, `right`, `inner`, `outer` 连接。

**`pd.concat()`**: 物理拼接。通常用于将结构相同的表上下堆叠（纵向）或左右拼接（横向）。

**`df.to_csv()`**: 导出为CSV文件（记得设置 `index=False` 通常是个好习惯）。
